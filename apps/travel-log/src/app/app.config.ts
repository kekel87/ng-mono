...
import { EntryEffects } from './log/stores/entry/entry.effects';
import { entryFeature } from './log/stores/entry/entry.feature';
...
export const appConfig: (runtimeConfig: RuntimeConfig) => ApplicationConfig = (runtimeConfig: RuntimeConfig) => ({
  providers: [
    ...
    provideEffects(RouterEffects, LogEffects, EntryEffects),
    provideState(layoutFeature),
    provideState(logFeature),
    provideState(entryFeature),
